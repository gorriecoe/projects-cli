...
module.exports = function(callback) {
  if (options.installer) {
    options.installer = objectPath.get(utility.config.Installers, options.installer);
    callback();
  } else {
    inquirer.prompt(question, function(answers) {
      options.installer = objectPath.get(utility.config.Installers, answers.installer);
      callback();
    });
  }
};
